...
class DaftarWisata(ctk.CTkFrame):
    def __init__(self, parent, callback_form):
        ...
        # --- KONFIGURASI LEBAR KOLOM (RESPONSIF) ---
        self.w_kota = 120
        self.w_htm = 110
        self.w_jam = 130
        self.w_rate = 80
    ...
    def tampilkan_halaman_daftar_wisata(self):
        ...
        # --- 2. AREA FILTER & SEARCH (LOGIC OFF) ---
        ...
        # Bind tetap ada tapi logic pass (kosong)
        self.teks_ui_nama_wisata.bind("<KeyRelease>", self.proses_pencarian)

        # Combo Filters & Button Tambah
        ...
        # Grid weight: Kolom Nama (0) fleksibel
        ...
        ctk.CTkLabel(table_header, text="RAT", width=self.w_rate, font=("Arial", 11, "bold"), text_color="#9CA3AF", anchor="w").grid(row=0, column=4, sticky="w")
        ctk.CTkLabel(table_header, text="", width=self.w_aksi).grid(row=0, column=5, sticky="e", padx=20)
    ...
    def proses_pencarian(self, event=None):
        """Logic search dimatikan sesuai permintaan (hanya tampilan)"""
        pass

    def refresh_tabel(self):
        """Memuat ulang data dari data_wisata.json"""
    ...
    def render_kartu_wisata(self, item):
        ...
        row.grid_columnconfigure(0, weight=1) # Nama Wisata fleksibel mengikuti layar
        
        # Ekstrak data JSON Bersarang
        ...
        lbl_nama = ctk.CTkLabel(info_frame, text=nama, font=("Arial", 13, "bold"), text_color="#1F2937", wraplength=300, justify="left", anchor="w")
        lbl_nama.pack(fill="x", anchor="w")
        ctk.CTkLabel(info_frame, text=tipe, font=("Arial", 11), text_color="#6B7280", anchor="w").pack(fill="x", anchor="w")

        # Kolom 1 - 4: Fixed Width agar rapi sejajar header
        ...
        # Kolom 5: Tombol Aksi (View, Edit, Delete)
        action_frame = ctk.CTkFrame(row, fg_color="transparent", width=self.w_aksi)
        action_frame.grid(row=0, column=5, sticky="e", padx=20)
        
        # View (Icon Mata)
        ctk.CTkButton(action_frame, text="👁️", width=30, fg_color="transparent", text_color="#10B981", hover_color="#E5E7EB").pack(side="left")
        
        # Edit (Redirect ke Form)
        ctk.CTkButton(action_frame, text="✏️", width=30, fg_color="transparent", text_color="#3B82F6", hover_color="#E5E7EB", 
                    command=lambda: self.callback_form("Edit", item)).pack(side="left")
        
        # Delete (Pop-up Konfirmasi)
        ctk.CTkButton(action_frame, text="🗑️", width=30, fg_color="transparent", text_color="#EF4444", hover_color="#FEE2E2", 
                    command=lambda: self.notif_konfirmasi(f"Hapus permanen {nama}?", item['id'])).pack(side="left")
