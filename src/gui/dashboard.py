...
def _load_foto(foto_list, w: int, h: int, fallback_hue: tuple) -> ctk.CTkImage:
    """Load gambar asli dari foto_list; fallback ke placeholder jika gagal."""
    for fname in (foto_list or []):
        if not fname or fname == "default.png":
            continue
        key = (fname, w, h)
...
class TopDestinasScroll(ctk.CTkFrame):
    ...
    def __init__(self, master, top_list: list, raw_data: list = None, **kwargs):
        super().__init__(master, fg_color="transparent", **kwargs)
    ...
    def _build(self):
        ...
        # Buat lookup foto dari raw_data berdasarkan nama
        foto_lookup: dict = {}
        for item in self._raw_data:
            idn = item.get("identitas", {})
            nama = idn.get("nama", "")
            fotos = idn.get("foto", [])
            if nama and fotos:
                foto_lookup[nama] = fotos
        ...
        for i, kat in enumerate(KATEGORI_TOP):
            ...
            if items:
                ...
                nama_best = best.get("nama", kat)
                fotos = foto_lookup.get(nama_best, [])
                thumb = _load_foto(fotos, 195, 115, hue)
...
class HalamanDashboard(ctk.CTkFrame):
    ...
    def _render_top_destinasi_section(self, metrik_data, data_stats):
        """Header + dropdown kategori & kota + kartu scroll horizontal."""
        ...
        kat_values = ["Semua Kategori"] + KATEGORI_TOP
        self._filter_kat_var = ctk.StringVar(value="Semua Kategori")
        filter_kat = ctk.CTkOptionMenu(
            header_frame,
            variable=self._filter_kat_var,
            values=kat_values,
            fg_color=C["card"],
            button_color=C["teal"],
            text_color=C["txt"],
        ...
        filter_kat.grid(row=0, column=1, padx=(8, 0), sticky="e")

        # ── Dropdown Semua Kota/Kabupaten ──
        # Gunakan bagian KEDUA dari alamat (setelah koma) = kabupaten/kota
        kota_set = set()
        for item in raw_data:
            alamat = (item.get("identitas", {}).get("alamat") or "").strip()
            if "," in alamat:
                kota_set.add(alamat.split(",", 1)[1].strip())
            elif alamat:
                kota_set.add(alamat)
        ...
        filter_kota.grid(row=0, column=2, padx=(8, 0), sticky="e")
    ...
    def _on_filter_changed(self, _=None):
        """Callback saat dropdown kategori atau kota berubah."""
        raw = self._cached_raw
        pilihan_kat  = self._filter_kat_var.get()
        ...
        if pilihan_kota == "Semua Kota / Kabupaten":
            ...
            for item in raw:
                alamat = (item.get("identitas", {}).get("alamat") or "").strip()
                kab = alamat.split(",", 1)[1].strip() if "," in alamat else alamat
                if kab == pilihan_kota:
                    filtered.append(item)

        # Hitung top_list dari data terfilter
        sorted_filtered = sorted(filtered, key=get_rating, reverse=True)
        top_list = []
        seen_kat: set = set()
        for item in sorted_filtered:
            idn = item.get("identitas", item)
            raw_tipe = idn.get("tipe") or idn.get("kategori") or "Umum"
            kat_display = KATEGORI_ALIAS.get(raw_tipe.lower().strip(), raw_tipe)
            # Filter kategori jika dipilih
            if pilihan_kat != "Semua Kategori" and kat_display != pilihan_kat:
                continue
            if kat_display not in seen_kat:
                seen_kat.add(kat_display)
                top_list.append({
                    "nama":          idn.get("nama", "-"),
                    "rating":        get_rating(item),
                    "kategori":      kat_display,
